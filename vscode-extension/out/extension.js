...
function activate(context) {
    ...
    // Register commands
    context.subscriptions.push(vscode.commands.registerCommand('kiro-agent.openChat', () => {
        if (!chatPanel) {
            chatPanel = new ChatPanel(context.extensionUri);
        }
        chatPanel.show();
    }));
    context.subscriptions.push(vscode.commands.registerCommand('kiro-agent.clearChat', () => {
        if (chatPanel) {
            chatPanel.clearChat();
        }
    }));
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider('kiro-agent.chatView', provider));
}
function deactivate() {
    if (chatPanel) {
        chatPanel.dispose();
...
class ChatViewProvider {
    ...
    async sendMessage(message) {
        ...
        try {
            if (enableStreaming) {
                ...
                // Non-streaming mode
                const response = await axios_1.default.post(`${serverUrl}/api/chat`, {
                    message,
                    session_id: this.sessionId,
                    stream: false
                });
    ...
    async approveChange(changeId) {
        ...
        const serverUrl = config.get('serverUrl', 'http://localhost:5000');
        try {
            const response = await axios_1.default.post(`${serverUrl}/api/approve`, {
                session_id: this.sessionId,
                change_id: changeId
            });
            this._view?.webview.postMessage({
    ...
    async rejectChange(changeId) {
        ...
        const serverUrl = config.get('serverUrl', 'http://localhost:5000');
        try {
            const response = await axios_1.default.post(`${serverUrl}/api/reject`, {
                session_id: this.sessionId,
                change_id: changeId
            });
            this._view?.webview.postMessage({
